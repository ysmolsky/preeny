...
int
preeny_socket_sync(int from, int to, int timeout)
{
...
void
preeny_socket_sync_loop(int from, int to)
{
    int r;
    ...
    while (!preeny_desock_shutdown_flag) {
        preeny_debug("loop %d to %d!\n", from, to);
        r = preeny_socket_sync(from, to, TIMEOUT);
        if (r < 0)
            return;
    }
}
...
void *
preeny_socket_sync_to_back(void *fd)
{
    int front_fd = (int)fd;
...
void *
preeny_socket_sync_to_front(void *fd)
{
    int front_fd = (int)fd;
    int back_fd = PREENY_SOCKET(front_fd);
...
accept(int sockfd, struct sockaddr *addr, socklen_t *addrlen)
{
    ...
    if (preeny_socket_threads_to_front[sockfd]) {
        int dupsock = dup(sockfd);
        preeny_debug("return accept %d\n", dupsock);
        return dupsock;
    } else
...
{
    if (preeny_socket_threads_to_front[sockfd]) {
        preeny_debug("Ignoring listen on %d backlog=%d\n", sockfd, backlog);
        return 0;
    } else
